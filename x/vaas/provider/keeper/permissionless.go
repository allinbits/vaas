...
func (k Keeper) FetchAndIncrementConsumerId(ctx context.Context) uint64 {
...
func (k Keeper) GetConsumerChainId(ctx context.Context, consumerId uint64) (string, error) {
	chainId, err := k.ConsumerChainId.Get(ctx, consumerId)
	if err != nil {
		return "", fmt.Errorf("failed to retrieve chain id for consumer id (%d): %w", consumerId, err)
	}
	return chainId, nil
}
...
func (k Keeper) ChainIdInUse(ctx context.Context, chainId string) (bool, error) {
	...
	for ; iter.Valid(); iter.Next() {
		chainIdinner, err := iter.Value()
		if err != nil {
			continue
		}
		if chainIdinner == chainId {
			return true, nil
		}
	}
	return false, nil
...
func (k Keeper) SetConsumerChainId(ctx context.Context, consumerId uint64, chainId string) {
	if err := k.ConsumerChainId.Set(ctx, consumerId, chainId); err != nil {
		panic(fmt.Errorf("failed to set chain id for consumer id (%d): %w", consumerId, err))
	}
}

// DeleteConsumerChainId deletes the chain id associated with this consumer id
func (k Keeper) DeleteConsumerChainId(ctx context.Context, consumerId uint64) {
	if err := k.ConsumerChainId.Remove(ctx, consumerId); err != nil {
		panic(fmt.Errorf("failed to delete chain id for consumer id (%d): %w", consumerId, err))
	}
}

// GetConsumerOwnerAddress returns the owner address associated with this consumer id
func (k Keeper) GetConsumerOwnerAddress(ctx context.Context, consumerId uint64) (string, error) {
	owner, err := k.ConsumerOwnerAddress.Get(ctx, consumerId)
	if err != nil {
		return "", fmt.Errorf("failed to retrieve owner address for consumer id (%d): %w", consumerId, err)
	}
	return owner, nil
}

// SetConsumerOwnerAddress sets the owner address associated with this consumer id
func (k Keeper) SetConsumerOwnerAddress(ctx context.Context, consumerId uint64, owner string) {
	if err := k.ConsumerOwnerAddress.Set(ctx, consumerId, owner); err != nil {
		panic(fmt.Errorf("failed to set owner address for consumer id (%d): %w", consumerId, err))
	}
}

// DeleteConsumerOwnerAddress deletes the owner address associated with this consumer id
func (k Keeper) DeleteConsumerOwnerAddress(ctx context.Context, consumerId uint64) {
	if err := k.ConsumerOwnerAddress.Remove(ctx, consumerId); err != nil {
		panic(fmt.Errorf("failed to delete owner address for consumer id (%d): %w", consumerId, err))
	}
}

// GetConsumerMetadata returns the registration record associated with this consumer id
func (k Keeper) GetConsumerMetadata(ctx context.Context, consumerId uint64) (types.ConsumerMetadata, error) {
	metadata, err := k.ConsumerMetadata.Get(ctx, consumerId)
	if err != nil {
		return types.ConsumerMetadata{}, fmt.Errorf("failed to retrieve metadata for consumer id (%d): %w", consumerId, err)
	}
	return metadata, nil
}

// SetConsumerMetadata sets the registration record associated with this consumer id
func (k Keeper) SetConsumerMetadata(ctx context.Context, consumerId uint64, metadata types.ConsumerMetadata) error {
	if err := k.ConsumerMetadata.Set(ctx, consumerId, metadata); err != nil {
		return fmt.Errorf("failed to set metadata for consumer id (%d): %w", consumerId, err)
	}
	return nil
}

// DeleteConsumerMetadata deletes the metadata associated with this consumer id
func (k Keeper) DeleteConsumerMetadata(ctx context.Context, consumerId uint64) {
	if err := k.ConsumerMetadata.Remove(ctx, consumerId); err != nil {
		panic(fmt.Errorf("failed to delete metadata for consumer id (%d): %w", consumerId, err))
	}
}

// GetConsumerInitializationParameters returns the initialization parameters associated with this consumer id
func (k Keeper) GetConsumerInitializationParameters(ctx context.Context, consumerId uint64) (types.ConsumerInitializationParameters, error) {
	params, err := k.ConsumerInitParams.Get(ctx, consumerId)
	if err != nil {
		return types.ConsumerInitializationParameters{}, fmt.Errorf("failed to retrieve initialization parameters for consumer id (%d): %w", consumerId, err)
	}
	return params, nil
}
...
func (k Keeper) SetConsumerInitializationParameters(ctx context.Context, consumerId uint64, parameters types.ConsumerInitializationParameters) error {
	...
	if err := k.ConsumerInitParams.Set(ctx, consumerId, parameters); err != nil {
		return fmt.Errorf("failed to set initialization parameters for consumer id (%d): %w", consumerId, err)
	}
	return nil
}
...
func (k Keeper) DeleteConsumerInitializationParameters(ctx context.Context, consumerId uint64) {
	if err := k.ConsumerInitParams.Remove(ctx, consumerId); err != nil {
		panic(fmt.Errorf("failed to delete initialization parameters for consumer id (%d): %w", consumerId, err))
	}
}

// GetConsumerPhase returns the phase associated with this consumer id
func (k Keeper) GetConsumerPhase(ctx context.Context, consumerId uint64) types.ConsumerPhase {
...
func (k Keeper) SetConsumerPhase(ctx context.Context, consumerId uint64, phase types.ConsumerPhase) {
	if err := k.ConsumerPhase.Set(ctx, consumerId, uint32(phase)); err != nil {
		panic(fmt.Errorf("failed to set consumer phase for consumer id (%d): %w", consumerId, err))
	}
}
