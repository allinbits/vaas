...
func (k Keeper) IsConsumerInDebt(ctx context.Context, consumerId uint64) bool {
	inDebt, err := k.ConsumerDebt.Get(ctx, consumerId)
	if err != nil {
		if errors.Is(err, collections.ErrNotFound) {
			return false
		}
		panic(fmt.Errorf("failed to read consumer debt status for %d: %w", consumerId, err))
	}
	return inDebt
}

func (k Keeper) SetConsumerInDebt(ctx context.Context, consumerId uint64, inDebt bool) {
	if err := k.ConsumerDebt.Set(ctx, consumerId, inDebt); err != nil {
		panic(fmt.Errorf("failed to set consumer debt status for %d: %w", consumerId, err))
	}
}

func (k Keeper) DeleteConsumerDebt(ctx context.Context, consumerId uint64) {
	if err := k.ConsumerDebt.Remove(ctx, consumerId); err != nil && !errors.Is(err, collections.ErrNotFound) {
		panic(fmt.Errorf("failed to delete consumer debt status for %d: %w", consumerId, err))
	}
}
...
func (k Keeper) UpdateConsumerDebtStatus(ctx sdk.Context, consumerId uint64, inDebt bool) {
