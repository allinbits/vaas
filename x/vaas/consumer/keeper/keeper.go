...
// always set. When it is absent anyway (the genesis block itself) fall back
// to the current block time, i.e. never stale until something arms it.
...
// (by a restart genesis, a received VSC, or an earlier arming) and at the
// genesis block itself: under
